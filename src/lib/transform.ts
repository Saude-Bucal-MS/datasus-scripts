...
import { fileExists, rmFile } from '../utils/fs.js';

/**
 * Transform data from a .dbf file into a sqlite database file.
 * @param filepath The path to the .dbc file.
 * @param opts Storage options.
 * @returns The path to the created sqlite database file.
 */
export type TransformFn = (filepath: string, opts?: { override?: boolean }) => Promise<string>;

function baseTransform(
  createSchemaFn: (db: Knex.Knex, tableName: string) => Promise<void>,
): TransformFn {
  return async (filepath, opts) => {
    const sqliteFilePath = filepath.replace('.dbf', '.sqlite');

    if (!fileExists(filepath)) {
      throw new FileNotFoundError(`File not found: ${filepath}`);
    }

    if (fileExists(sqliteFilePath)) {
      if (!opts?.override) {
        throw new DataAlreadyExistsError(
          `Database file ${sqliteFilePath} already exists. Use override option to replace it.`,
        );
      }
      rmFile(sqliteFilePath);
    }
    ...
    await db
      .transaction(async (trx) => {
        ...
        await createSchemaFn(trx, tableName);
        for await (const record of await DBFFile.open(filepath, { encoding: 'ISO-8859-1' })) {
          await trx.table(tableName).insert(record);
        }
      })
      .finally(() => db.destroy());
...
/**
 * Transform function for PA files.
 */
export const transformPA: TransformFn = baseTransform((trx, tableName) =>
  trx.schema.createTable(tableName, (table) => {
    ...
    table
      .string('PA_UFMUN', 6)
      .comment('Unidade da Federação + Código do Município onde está localizado o estabelecimento');
    ...
    table.string('PA_CMP', 6).comment('Data da Realização do Procedimento / Competência (AAAAMM)');
    ...
    table.string('IDADEMIN', 3).comment('Idade mínima do paciente para realização do procedimento');
    table.string('IDADEMAX', 3).comment('Idade máxima do paciente para realização do procedimento');
    table
      .string('PA_FLIDADE', 1)
      .comment(
